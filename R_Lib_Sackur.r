...
roc2 <- function(conf, acc, n=7, plot=TRUE, add=FALSE, col=NULL, title=''){
  ...
  if (plot){
    if (!add){
      if (title==''){title=round(auc, 3)}
      plot(fas, hits, type='b', cex=2, lty=1, xlim=c(0, 1), ylim=c(0, 1),
           xlab='', ylab='', col=col,
           main=title, pch=19)
      lines(0:1, 0:1, lty=2)
    } else {
      lines(fas, hits, type='b', cex=2, lty=1, col=col, pch=19)
    }
  }
 return(list(auc2=auc, B2=B))
}
...
readFastDM <- function(file, dv='v', factors, flevels){ #changed dv for dvs everywhere
#readFastDM <- function(file, dvs, factors, flevels){
    ...
    #assign(dvs, unlist(fdm[,grep(paste('^',dvs, '_', sep=''), cols)]))
    i <- 0
